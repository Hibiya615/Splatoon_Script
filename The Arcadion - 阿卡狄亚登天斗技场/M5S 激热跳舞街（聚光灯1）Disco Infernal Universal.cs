...
public unsafe class M5S_激热跳舞街_聚光灯1_Disco_Infernal_Universal : SplatoonScript
{
    public override HashSet<uint>? ValidTerritories { get; } = [1257];

    public override Metadata? Metadata => new(7, "NightmareXIV,南雲鉄虎本地化");
    ...
    {
        Controller.RegisterElementFromCode("Prepare", "{\"Name\":\"\",\"refX\":0,\"refY\":0,\"radius\":2.0,\"Donut\":0.5,\"color\":3355508694,\"fillIntensity\":0.8,\"overlayBGColor\":3355443200,\"overlayTextColor\":4278255103,\"overlayFScale\":1.0,\"thicc\":4.0,\"overlayText\":\"倒计时: X\"}");
        Controller.RegisterElementFromCode("Go", "{\"Name\":\"\",\"refX\":0,\"refY\":0,\"radius\":2.0,\"Donut\":0.5,\"color\":3355508509,\"fillIntensity\":0.8,\"overlayBGColor\":3355443200,\"overlayTextColor\":4278255389,\"overlayFScale\":1.4,\"thicc\":4.0,\"overlayText\":\"倒计时: X\",\"tether\":true}");
        for(int i = 0; i < 8; i++)
        {
    ...
    public override void OnUpdate()
    {
        ...
        if(MechanicStarted && TargetedTile != null && BasePlayer.StatusList.TryGetFirst(s => s.StatusId == Debuff, out var d))
        {
            ...
            e.Enabled = true;
            e.overlayText = $"倒计时: {d.RemainingTime:F1}";
    ...
    public override void OnSettingsDraw()
    {
        ...
        {
            ImGuiEx.Text(EColor.GreenBright, "远程配置有效");
        }
        else if(C.SelectedTiles.Count(x => !x.IsRanged()) == 4 && C.SelectedTiles.Count == 4)
        {
            ImGuiEx.Text(EColor.GreenBright, "近战配置有效");
        }
        else
        {
            ImGuiEx.Text(EColor.RedBright, "当前配置无效。请根据攻略在下方图示里选择 2 个（远程）或 4 个（近战）你可能会去的位置：");
        }
        for(int i = 0; i < 8; i++)
        {
            for(int k = 0; k < 8; k++)
        ...
        ImGui.Text("""
            你应该在倒计时约6s时，移动至旁边的格子待机，
            在倒计时约2.5s时，再走进安全格
            """
            );
        ImGui.Checkbox("显示全部的安全格", ref C.DisplayAllSafe);
