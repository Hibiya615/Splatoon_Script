...
public unsafe class EX4_泽莲尼娅歼殛战_凌空错_远近刀_Escelons_Fall : SplatoonScript
{
    public override HashSet<uint>? ValidTerritories { get; } = [1271];

    public override Metadata? Metadata => new(7, "NightmareXIV, Redmoonwow, 南雲鉄虎本地化");
    ...
    public override void OnSetup()
    {
        ...
        Controller.RegisterElementFromCode("InIncorrect", "{\"Name\":\"\",\"type\":1,\"radius\":1.0,\"Filled\":false,\"fillIntensity\":0.5,\"overlayTextColor\":4278190335,\"thicc\":4.0,\"overlayText\":\"靠近\",\"refActorType\":1,\"refActorTetherTimeMin\":0.0,\"refActorTetherTimeMax\":0.0}");
        Controller.RegisterElementFromCode("InCorrect", "{\"Name\":\"\",\"type\":1,\"radius\":1.0,\"color\":3355508480,\"Filled\":false,\"fillIntensity\":0.5,\"overlayTextColor\":3355508480,\"thicc\":4.0,\"overlayText\":\"> 靠近 <\",\"refActorType\":1,\"refActorTetherTimeMin\":0.0,\"refActorTetherTimeMax\":0.0}");
        Controller.RegisterElementFromCode("OutIncorrect", "{\"Name\":\"\",\"type\":1,\"radius\":1.0,\"color\":3372155135,\"Filled\":false,\"fillIntensity\":0.5,\"overlayTextColor\":3372155135,\"thicc\":4.0,\"overlayText\":\"远离\",\"refActorType\":1,\"refActorTetherTimeMin\":0.0,\"refActorTetherTimeMax\":0.0}");
        Controller.RegisterElementFromCode("OutCorrect", "{\"Name\":\"\",\"type\":1,\"radius\":1.0,\"color\":3355508480,\"Filled\":false,\"fillIntensity\":0.5,\"overlayTextColor\":3355508480,\"thicc\":4.0,\"overlayText\":\"远离\",\"refActorType\":1,\"refActorTetherTimeMin\":0.0,\"refActorTetherTimeMax\":0.0}");
    }

    public override void OnSettingsDraw()
    {
        ImGuiEx.Text($"自己的引导轮次：先近引导");
        ImGuiEx.HelpMarker($"如果第一次是近引导，你该引导哪一次？");
        ImGuiEx.RadioButtonBool("第一次 (先靠近)", "第二次 (先远离)", ref C.TakeFirstIfClose);
        ImGuiEx.Text($"自己的引导轮次：先远引导");
        ImGuiEx.HelpMarker($"如果第一次是远引导，你该引导哪一次？");
        ImGuiEx.RadioButtonBool("第一次 (先远离)##2", "第二次 (先靠近)##2", ref C.TakeFirstIfFar);
        ImGui.Separator();
        ImGui.SetNextItemWidth(150f);
        ImGuiEx.SliderInt("延迟(ms)", ref C.Delay, 0, 1000);
        ImGuiEx.HelpMarker("延迟有助于将脚本与攻击动画同步。如果你希望尽快看到安并全移动，推荐将其设置为 0。");
        ImGui.Checkbox("完全不处理第二个机制 (适用于 \"无脑\" 打法)", ref C.IgnoreSecond);
        if(ImGui.CollapsingHeader("Debug"))
        {
            ImGui.Checkbox("调整阶段", ref AdjustPhase);
            ImGui.Checkbox("TH震惊点名", ref THShockTargeted);
